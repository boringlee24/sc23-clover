...
MIG = FILE.parents[2]  # YOLOv5 root directory
YOLO = FILE.parents[1]
...
logging.config.fileConfig(fname=f'{YOLO}/logging.conf', disable_existing_loggers=False)
...
with open(f'{YOLO}/master.json') as f:
    master = json.load(f)
URL = master['master']
...
models = ['yolov5l', 'yolov5x', 'yolov5x6']

partitions = [0]#[6, 17]#[1, 3, 4]#, 
