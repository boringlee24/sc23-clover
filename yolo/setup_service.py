...
def configure():
    ...
    # 'start': [gpuid, mig_config]
    # 'multi_start': [num_gpu, mig_config]
    # 'weights': xx}
...
def start_multigpu_service(num_gpu, mig_config, weight_list):
    ...
    for k in range(num_gpu):
        ...
        for i, (device, weight) in enumerate(zip(mig_list, weight_list)):
            cmd = f'CUDA_VISIBLE_DEVICES={device} python app_yolo.py --weights={weight} --port 500{idx}' 
            out_file = f'/scratch/li.baol/carbon_logs/yolo{idx}.out'
            err_file = f'/scratch/li.baol/carbon_logs/yolo{idx}.err'
...
def start_mixed_service(gpuid, mig_config, weight_list):
    ...
    tracker_ports = {}
    for i, (device, weight) in enumerate(zip(mig_list, weight_list)):
        cmd = f'CUDA_VISIBLE_DEVICES={device} python app_yolo.py --weights={weight} --port 500{i}'        
        out_file = f'/scratch/li.baol/carbon_logs/yolo{i}.out'
        err_file = f'/scratch/li.baol/carbon_logs/yolo{i}.err'
        with open(out_file, 'w+') as out, open(err_file, 'w+') as err:
            proc = subprocess.Popen([cmd], shell=True, stdout=out, stderr=err)
...
def start_service(gpuid, mig_config, weights):
    ...
    for i, device in enumerate(mig_list):
        cmd = f'CUDA_VISIBLE_DEVICES={device} python app_yolo.py --weights={weights} --port 500{i}'        
        out_file = f'/scratch/li.baol/carbon_logs/yolo{i}.out'
        err_file = f'/scratch/li.baol/carbon_logs/yolo{i}.err'
        with open(out_file, 'w+') as out, open(err_file, 'w+') as err:
            proc = subprocess.Popen([cmd], shell=True, stdout=out, stderr=err)
