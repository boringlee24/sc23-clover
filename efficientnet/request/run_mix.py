...
logging.config.fileConfig(fname=f'{EFFICIENTNET}/logging.conf', disable_existing_loggers=False)
...
parser.add_argument('--weights', type=str, default='b1', help='efficientnet variants b0 - b7')
parser.add_argument('--mode', type=str, help='operation mode: term/start/config', default='term')            
args = parser.parse_args()

with open(f'{EFFICIENTNET}/master.json') as f:
    master = json.load(f)
URL = master['master']

def get_model_code(models):
    s = ''
    for model in models:
        s += model[-1]
    return s
