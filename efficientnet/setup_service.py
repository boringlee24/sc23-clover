...
def configure():
    ...
    if 'term' in req_json:
        ...
        for gpuid in range(num_gpu):
            config_gpu(gpuid, mig_config)
        return make_response(jsonify(output), 201)
    elif 'start' in req_json:
        gpuid, mig_config, weights = req_json['start']
        start_service(gpuid, mig_config, weights)
        return make_response(jsonify(output), 201)
...
def start_multigpu_service(num_gpu, mig_config, weight_list):
    ...
    for k in range(num_gpu):
        ...
        for i, (device, weight) in enumerate(zip(mig_list, weight_list)):
            cmd = f'CUDA_VISIBLE_DEVICES={device} python app_efficientnet.py --weights={weight} --port 500{idx}'       
            out_file = f'/scratch/li.baol/carbon_logs/efficientnet{idx}.out'
            err_file = f'/scratch/li.baol/carbon_logs/efficientnet{idx}.err'
...
def start_mixed_service(gpuid, mig_config, weight_list):
    ...
    tracker_ports = {}
    for i, (device, weight) in enumerate(zip(mig_list, weight_list)):
        cmd = f'CUDA_VISIBLE_DEVICES={device} python app_efficientnet.py --weights={weight} --port 500{i}'        
        out_file = f'/scratch/li.baol/carbon_logs/efficientnet{i}.out'
        err_file = f'/scratch/li.baol/carbon_logs/efficientnet{i}.err'
        with open(out_file, 'w+') as out, open(err_file, 'w+') as err:
            proc = subprocess.Popen([cmd], shell=True, stdout=out, stderr=err)
...
def start_service(gpuid, mig_config, weights):
    with open(f'{MIG_DIR}/mig_device_autogen.json') as f:
        device_json = json.load(f)

    mig_list = device_json[hostname][f'gpu{gpuid}'][mig_config]        
    
    service_ports = {} # {pid: url}
    tracker_ports = {}
    for i, device in enumerate(mig_list):
        cmd = f'CUDA_VISIBLE_DEVICES={device} python app_efficientnet.py --weights={weights} --port 500{i}'        
        out_file = f'/scratch/li.baol/carbon_logs/efficientnet{i}.out'
        err_file = f'/scratch/li.baol/carbon_logs/efficientnet{i}.err'
        with open(out_file, 'w+') as out, open(err_file, 'w+') as err:
            proc = subprocess.Popen([cmd], shell=True, stdout=out, stderr=err)
        service_ports[proc.pid] = f'http://{ip_addr}:500{i}'
