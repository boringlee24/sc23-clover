...
def disable_mig(gpuid):
    cmd = f'sudo nvidia-smi -i {gpuid} -mig 0'
    p = subprocess.Popen([cmd], shell=True)
    p.wait()
...
def reset_mig(gpu):
    ...
    while not success:
        ...
        if 'Unable to destroy' not in read:
            ...
            print('Trying again to destroy MIG instances...')
            time.sleep(0.5)
